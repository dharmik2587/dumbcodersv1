...
export const POST = withApiHandler(
  { api: 'teams', operation: 'createTeam', requireAuth: true },
  async ({ req, userId }) => {
    if (!hasCoreDatabase()) return failure('NOT_CONFIGURED', 'Database is not configured.', 503);
    ...
    const [team] = await db.insert(teams)
      .values({ ...parsed.data, hackathonId: parsed.data.hackathonId ?? null, description: parsed.data.description ?? null, leaderId: userId! })
      .returning();

    if (!team) return failure('CREATE_FAILED', 'Could not create the team.', 500);

    await db.insert(teamMembers).values({ teamId: team.id, userId: userId!, role: 'leader' });
