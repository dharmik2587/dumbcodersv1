...
function SignInContent() {
  ...
  const handleForgotPassword = async (e: FormEvent) => {
    ...
    setForgotSuccess('');
    try {
      const { resetPasswordForEmail } = await import('@/client/lib/auth');
      ...
      <Reveal className="relative z-10 w-full max-w-[440px]">
        <Panel className="overflow-hidden">
          <div className="border-b border-line px-8 py-6 text-center">
            <h1 className="display text-2xl font-medium text-fg">
              {forgotMode ? 'Reset password' : 'Welcome back'}
            </h1>
            <p className="mt-2 text-[14px] text-fg2">
              {forgotMode ? 'We will send you a link to reset your password' : 'Sign in to your HackMate account'}
            </p>
          </div>
            ...
            {forgotMode ? (
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="font-mono text-[10.5px] uppercase tracking-wider text-fg2">Your Email address</label>
                  <input
                    type="email"
                    required
                    value={email}
                ...
                <div className="text-center pt-2">
                  <button
                    ...
                    className="text-[12.5px] text-fg2 hover:text-fg underline"
                  >
                    Back to sign in
                  </button>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-[11px] text-accent hover:text-accent-ink transition-colors"
                    >
                      Forgot password?
                    </button>
                  </div>
